...
void setup(){
  ...
  // make sure addon is not activated
  // if so flash LED and wait
  while(receiver_input[5] > 1500) {
    start++; 
    //We don't want the esc's to be beeping annoyingly. So let's give them a 1000us puls while waiting for the receiver inputs.
    PORTD |= B11110000;                                                     //Set digital port 4, 5, 6 and 7 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTD &= B00001111;                                                     //Set digital port 4, 5, 6 and 7 low.
    delay(3);                                                               //Wait 3 milliseconds before the next loop.
    if(start == 65){                                                        //Every 125 loops (500ms).
      digitalWrite(12, !digitalRead(12));                                   //Change the led status.
      start = 0;                                                            //Start again at 0.
    }
  }
...
ISR(PCINT0_vect){
  ...
  if(PINB & B00100000 ){                                                    //Is input 13 high?
    if(last_channel_5 == 0){                                                //Input 13 changed from 0 to 1.
      ...
      timer_5 = current_time;                                               //Set timer_45 to current_time.
    }
  }
